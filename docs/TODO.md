...
| 系统重构   | 用户系统数据库重构设计                 | 待定   | P0     | 已完成   | 将单表用户设计重构为多表继承模式
| 系统重构   | 创建基础用户表和角色关联表             | 待定   | P0     | 已完成   | 创建Users表、Roles表和UserRoles关联表
| 系统重构   | 创建用户角色扩展表                     | 待定   | P0     | 已完成   | 创建Customers、Doctors、Consultants、Operators、Administrators扩展表
| 系统重构   | 实现数据迁移脚本                       | 待定   | P0     | 已完成   | 将现有用户数据按角色分类迁移到新表结构
| 系统重构   | 重构用户认证和授权系统                 | 待定   | P0     | 已完成   | 实现基于角色的访问控制(RBAC)
| 系统重构   | 更新用户管理相关API                    | 待定   | P0     | 已完成   | 修改/api/app/api/v1/endpoints/users.py
| 系统重构   | 实现顾客自主注册流程                   | 待定   | P0     | 已完成   | 公开注册默认为顾客角色
| 系统重构   | 实现用户角色切换功能                   | 待定   | P0     | 已完成   | 支持拥有多角色的用户在角色间切换
| 系统重构   | 更新前端用户状态管理                   | 待定   | P0     | 已完成   | 修改AuthContext适配新的用户角色结构
| 系统重构   | 更新管理员端用户管理界面               | 待定   | P0     | 已完成   | 适配新的用户角色结构
| 系统重构   | 实现用户权限验证中间件                 | 待定   | P0     | 已完成   | 基于新角色结构的权限控制
| 系统重构   | 编写用户系统单元测试                   | 待定   | P0     | 已完成   | 确保重构后功能正确性
...
| 聊天系统集成         | 对接前端聊天窗口组件                   | 待定   | P0     | 进行中   | 已添加WebSocket连接初始化和消息处理
