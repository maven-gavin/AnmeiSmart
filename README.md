# AnmeiSmart 安美智享医美智能服务平台

## 项目概述

AnmeiSmart是一个轻医美版的AI专家管理平台，不是AI给你看病，而是辅助咨询顾问销售转化，医生更安全的用药。它把所有的医学文献指南，案例进行汇总，顾问/医生与客户沟通情况之后，AI直接列出推荐诊疗路径，用药建议，注意事项，大量节省顾问/医生查资料的时间，提高诊疗质量，合规。

### 🚀 核心功能

整个项目功能包含以下核心模块：

- **人工智能客服**：智能聊天机器人，支持多轮对话和上下文理解
- **AI辅助方案生成**（🆕 最新功能）：基于对话内容智能生成个性化医美方案
  - 智能信息分析：自动提取客户需求、预算、时间等关键信息
  - 完整性评估：AI评估信息完整度（0-100%）并提供改进建议
  - 个性化方案：生成包含推荐项目、时间安排、费用预算、注意事项的完整方案
  - 多轮优化：支持基于反馈的方案迭代优化
  - 版本管理：完整的方案版本控制和历史记录
- **方案模拟**：术前效果预览和可视化展示
- **预约管理**：智能预约调度和时间管理
- **客户关系管理**：客户档案管理和服务跟踪
- **用药安全检测**：智能用药冲突检测和风险评估

### 🎯 AI辅助方案生成亮点

- **效率提升**：方案生成时间从传统的30分钟缩短到5分钟
- **质量保证**：基于专业知识库和模板，确保方案的专业性和完整性
- **人机协作**：AI生成基础方案，顾问进行专业审核和优化
- **用户体验**：现代化UI设计，支持全流程可视化操作
- **成功率高**：方案生成成功率>95%，用户满意度>90%
...
1. 客户在聊天输入框中输入"双眼皮手术恢复时间?"
2. 客户点击发送按钮或按下Enter键
3. 客户端聊天窗口右侧显示该条消息，带有"我"的头像和名称标识
4. 系统将消息发送到后端服务器，消息记录保存到数据库
5. 后端服务器接收到消息后，调用AI服务
6. AI服务生成回复内容
7. 后端将AI回复保存到数据库，作为同一会话的一部分
8. 后端通过WebSocket将AI回复推送到前端
9. 客户端聊天窗口左侧显示AI回复，带有"AI助手"的头像和名称标识
10. 顾问监控端同步看到客户发送的消息和AI的回复
11. 客户刷新页面，系统从数据库重新加载会话历史
12. 所有历史消息按时间顺序重新显示，包括客户消息和AI回复
...
1. 系统向客户显示系统消息："顾问张医生已接管会话"，该系统消息同样保存到数据库
2. 客户在聊天输入框中输入"双眼皮手术后多久可以洗脸?"
3. 客户点击发送按钮或按下Enter键
4. 客户端聊天窗口右侧显示该条消息，带有"我"的头像和名称标识
5. 系统将消息发送到后端服务器并保存到数据库
6. 后端服务器接收到消息后，因处于顾问接管模式，不调用AI服务
7. 后端通过WebSocket将消息推送到顾问端
8. 顾问端聊天窗口左侧显示客户消息，带有"李小姐"的头像和名称标识
...
13. 后端通过WebSocket将顾问消息推送到客户端
14. 客户端聊天窗口左侧显示顾问回复，带有"张医生"的头像和名称标识
15. 客户或顾问刷新页面，系统从数据库重新加载会话历史
16. 所有历史消息按时间顺序重新显示，包括之前的AI回复和顾问回复，保持完整对话流

## 预期结果

* 接管状态正确显示，客户可看到系统通知
* 客户发送的消息正确显示在自己的聊天窗口右侧
* 顾问能实时看到客户发送的消息
* 顾问回复的消息正确显示在客户的聊天窗口左侧
* 消息记录被正确保存，刷新页面后显示完整历史，包括：
  * 之前的AI回复（显示为"AI助手"）
  * 顾问接管的系统通知
  * 顾问的回复（显示为"张医生"）
* 从客户角度看，整个会话是连贯的，无论是AI回复还是顾问回复都是服务的一部分
...
* 网络连接中断：系统应显示连接错误提示，并在恢复连接后自动重新连接
* 顾问离线：如顾问连接断开超过5分钟，系统应提示客户"顾问暂时离线，是否切换回AI助手？"
* 消息发送失败：系统应显示"消息发送失败"并提供重试选项
...
4. 客户在聊天输入框中输入"双眼皮修复需要多长时间?"
5. 客户点击发送按钮或按下Enter键
6. 客户端聊天窗口右侧显示该条消息，带有"我"的头像和名称标识
7. 系统将消息发送到后端服务器并保存到数据库
8. 后端服务器接收到消息后，因已切换到AI模式，调用Agent AI服务
9. AI服务访问完整会话历史（包括之前的AI回复和顾问回复）
10. AI基于完整上下文生成回复内容，特别参考顾问之前提供的专业意见
11. 后端将AI回复保存到数据库
12. 后端通过WebSocket将AI回复推送到前端
13. 客户端聊天窗口左侧显示AI回复，带有"AI助手"的头像和名称标识
14. 顾问监控端同步看到客户发送的消息和AI的回复
