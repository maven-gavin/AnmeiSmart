...
test.describe('聊天功能 - 顾问应答', () => {
  ...
  test('客户发送文本消息，顾问正常应答', async ({ browser }) => {
    ...
    try {
      console.log('开始测试: 客户发送文本消息，顾问正常应答');
      
      // 1. 客户登录（在真实场景中，客户通过UI登录系统）
      ...
      // 7. 验证客户消息显示在客户界面（客户在自己的界面看到已发送的消息）
      console.log('步骤7: 验证客户消息显示在客户界面');
      ...
      // 8. 验证顾问能看到客户消息（顾问在自己的界面看到客户发送的消息）
      console.log('步骤8: 验证顾问能看到客户消息');
      ...
      // 12. 验证顾问回复在客户界面显示（客户收到顾问的回复）
      console.log('步骤12: 验证顾问回复在客户界面显示');
      ...
      // 13. 刷新客户页面，验证历史记录持久化（消息记录应该保存在服务器）
      console.log('步骤13: 刷新客户页面，验证历史记录持久化');
      await customerPage.reload();
      await waitForChatReady(customerPage);
      
      // 14. 验证刷新后客户消息仍然存在
      console.log('步骤14: 验证刷新后客户消息仍然存在');
      await expect(
      ...
      expect(customerMessageFound, '客户消息应该存在于数据库中').toBeTruthy();
      ...
      console.log('测试完成: 客户发送文本消息，顾问正常应答 ✅');
    } catch (error) {
