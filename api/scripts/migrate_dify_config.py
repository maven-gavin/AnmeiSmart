...
def migrate_env_to_db():
    ...
    try:
        # 检查是否已有配置
        existing_config = db.query(AgentConfig).first()
        if existing_config:
            print("⚠️  数据库中已存在Agent配置，跳过迁移")
        ...
        # 重载AI Gateway配置
        try:
            from app.services.agent_config_service import reload_ai_gateway_with_new_config
...
def create_sample_config():
    ...
    try:
        ...
        existing_config = db.query(AgentConfig).first()
        if existing_config:
            print("⚠️  数据库中已存在Agent配置，跳过创建")
            return
        
        # 创建示例配置
        sample_config = AgentConfig(
            config_name="示例Agent配置",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Agent配置迁移脚本")
