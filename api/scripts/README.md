...
# 不使用迁移系统，直接创建表
python init_db.py --no-migrations
```
...
# 创建新的迁移（自动检测变更）
python migration.py create --message "添加新字段"

# 手动创建迁移（不自动检测）
python migration.py create --message "自定义迁移" --manual

# 应用迁移到最新版本
python migration.py upgrade

# 回滚到指定版本
python migration.py downgrade --revision 1a2b3c4d
...
# 2. 创建迁移
python migration.py create --message "变更描述"
