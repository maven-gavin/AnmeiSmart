...
class RoleService:
    ...
    def create_role(self, role_in: RoleCreate) -> Role:
        ...
        if self.get_role_by_name(role_in.name):
            raise BusinessException("角色名称已存在", code=ErrorCode.RESOURCE_ALREADY_EXISTS)
    ...
    def update_role(self, role_id: str, role_data: RoleUpdate) -> Role:
        ...
        if not role:
            raise BusinessException("角色不存在", code=ErrorCode.RESOURCE_NOT_FOUND)
    ...
    def delete_role(self, role_id: str) -> bool:
        ...
        if role.is_system:
             raise BusinessException("系统角色无法删除", code=ErrorCode.OPERATION_NOT_ALLOWED)
