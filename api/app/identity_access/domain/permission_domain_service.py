...
class PermissionDomainService:
    ...
    async def check_user_permission(
        ...
        # 检查用户状态
        if not userEntity.can_access_system():
            return False
        ...
        # 回退到传统权限检查
        user_permissions = await self._get_user_permissions(userEntity)
    ...
    async def check_user_role(self, user_id: str, role_name: str) -> bool:
        ...
        # 回退到传统角色检查
        return userEntity.has_role(role_name)
    ...
    async def switch_user_role(
        ...
        # 检查用户是否有目标角色
        if not userEntity.has_role(target_role):
            return False
    ...
    async def _get_user_permissions(self, user: UserEntity) -> Set[str]:
        """获取用户所有权限"""
        permissions = set()
    ...
    async def get_user_primary_role(self, user_id: str) -> str:
        ...
        primary_role = userEntity.get_primary_role()
    ...
    async def is_medical_staff(self, user_id: str) -> bool:
        ...
        return userEntity.is_medical_staff()
    ...
    async def get_role_hierarchy_level(self, user_id: str) -> int:
        ...
        max_level = 0
        for role_name in userEntity.roles:
