...
class AgentConfigCreate(BaseModel):
    ...
    capabilities: Optional[Dict[str, Any]] = Field(None, description="Agent运行时能力配置(JSON)")
    apiKey: str = Field(..., description="LLM API密钥", min_length=1)
    baseUrl: str = Field(default_factory=lambda: settings.AGENT_DEFAULT_BASE_URL, description="LLM API Base URL")
...
class AgentConfigUpdate(BaseModel):
    ...
    agentType: Optional[str] = Field(None, description="智能体类型（chat/agent/workflow）")
    capabilities: Optional[Dict[str, Any]] = Field(None, description="Agent运行时能力配置(JSON)")
    apiKey: Optional[str] = Field(None, description="LLM API密钥", min_length=1)
    baseUrl: Optional[str] = Field(None, description="LLM API Base URL")
...
class AgentConfigInfo(BaseModel):
    ...
    capabilities: Optional[Dict[str, Any]] = Field(None, description="运行时能力配置", alias="capabilities")
    baseUrl: str = Field(..., description="LLM API Base URL", alias="base_url")
