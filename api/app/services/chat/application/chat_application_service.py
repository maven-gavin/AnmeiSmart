...
from app.services.chat.domain.interfaces import (
...
from app.services.chat.converters.conversation_converter import ConversationConverter
from app.services.chat.converters.message_converter import MessageConverter
...
class ChatApplicationService(IChatApplicationService):
    ...
    async def create_conversation_use_case(
        ...
        try:
            ...
            # 5. 转换为响应Schema
            conversation_info = ConversationConverter.to_response(saved_conversation)
    ...
    async def get_conversation_by_id_use_case(
        ...
        """获取指定会话用例"""
        try:
            logger.info(f"应用服务：获取会话 - conversation_id={conversation_id}, user_id={user_id}")
    ...
    async def update_conversation(self, conversation_id: str, user_id: str, updates: dict) -> Optional[ConversationInfo]:
        """更新会话用例"""
        logger.info(f"应用服务：更新会话 - conversation_id={conversation_id}, user_id={user_id}")
    ...
    async def delete_conversation(self, conversation_id: str, user_id: str) -> bool:
        """删除会话用例"""
        logger.info(f"应用服务：删除会话 - conversation_id={conversation_id}, user_id={user_id}")
        ...
        try:
            ...
            if self.broadcasting_service:
                try:
                    ...
                    logger.error(f"消息广播失败: message_id={saved_message.id}, error={e}")
