...
class ConversationDomainService(IConversationDomainService):
    ...
    async def verify_access(self, conversation_id: str, user_id: str, user_role: str) -> bool:
        """验证用户对会话的访问权限 - 领域逻辑"""
        logger.info(f"领域服务：验证访问权限 - conversation_id={conversation_id}, user_id={user_id}, role={user_role}")
    ...
    async def verify_ownership(self, conversation_id: str, user_id: str) -> bool:
        """验证用户是否为会话所有者 - 领域逻辑"""
        logger.info(f"领域服务：验证所有权 - conversation_id={conversation_id}, user_id={user_id}")
    ...
    async def archive_conversation(self, conversation_id: str, user_id: str) -> Conversation:
        """归档会话 - 领域逻辑"""
        logger.info(f"领域服务：归档会话 - conversation_id={conversation_id}, user_id={user_id}")
    ...
    async def pin_conversation(self, conversation_id: str, user_id: str, is_pinned: bool) -> Conversation:
        """置顶/取消置顶会话 - 领域逻辑"""
        logger.info(f"领域服务：置顶会话 - conversation_id={conversation_id}, user_id={user_id}, is_pinned={is_pinned}")
    ...
    async def mark_as_read(self, conversation_id: str, user_id: str) -> Optional[Conversation]:
        """标记会话为已读 - 领域逻辑"""
        logger.info(f"领域服务：标记已读 - conversation_id={conversation_id}, user_id={user_id}")
