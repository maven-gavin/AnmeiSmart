...
class ConsultationDomainService:
    ...
    def can_complete_consultation(self, consultation: Consultation) -> bool:
        """检查是否可以完成咨询"""
        # 检查咨询状态
        ...
        # 检查是否已分配顾问
        if not consultation.consultant_id:
            return False
    ...
    def can_cancel_consultation(self, consultation: Consultation) -> bool:
        """检查是否可以取消咨询"""
        # 检查咨询状态
    ...
    def calculate_consultation_duration(
        ...
        if consultation.status == ConsultationStatus.COMPLETED:
            duration = consultation.updated_at - consultation.created_at
            return int(duration.total_seconds() / 60)
        
        # 计算从创建到当前的时间
        from datetime import datetime
        duration = datetime.utcnow() - consultation.created_at
        return int(duration.total_seconds() / 60)
    ...
    def get_consultation_summary(self, consultation: Consultation) -> Dict[str, Any]:
        """获取咨询摘要信息"""
        return {
            "id": consultation.id,
