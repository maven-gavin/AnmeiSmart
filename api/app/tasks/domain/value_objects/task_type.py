...
class TaskType(str, Enum):
    ...
    def get_medical_task_types(cls) -> List[str]:
        """获取医疗相关任务类型"""
        return [cls.PRESCRIPTION_REVIEW, cls.MEDICAL_CONSULTATION]
    
    @classmethod
    def get_consultant_task_types(cls) -> List[str]:
        """获取顾问相关任务类型"""
        return [cls.NEW_USER_RECEPTION, cls.CONSULTATION_UPGRADE]
    
    @classmethod
    def get_operator_task_types(cls) -> List[str]:
        """获取运营相关任务类型"""
        return [cls.SYSTEM_MAINTENANCE, cls.USER_FEEDBACK]
    
    @classmethod
    def get_task_type_metadata(cls, task_type: str) -> Dict[str, Any]:
        """获取任务类型元数据"""
        metadata = {
            cls.NEW_USER_RECEPTION: {
                "name": "新用户接待",
                "description": "新注册用户需要顾问主动联系提供咨询服务",
                "default_priority": "medium",
                "auto_assign": False
            },
            cls.CONSULTATION_UPGRADE: {
                "name": "咨询升级",
                "description": "用户咨询需要升级到更高级别服务",
                "default_priority": "high",
                "auto_assign": False
            },
            cls.PRESCRIPTION_REVIEW: {
                "name": "处方审核",
